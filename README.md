...
#### Mac 桌面版
下载最新的 `.dmg` 文件：
👉 [GitHub Releases](https://github.com/yourusername/insight-asset-os/releases)
...
- **macOS 11+**（桌面 app）
- **SQLite 3**（内置）
- **LLM API key**（可选，DeepSeek / OpenAI / 通义千问 / Moonshot 都支持）
...
A: 任何 OpenAI 兼容 API 都行：OpenAI / 通义千问 / Moonshot / 智谱 GLM。在 `/onboarding` 选"自定义"填 base URL + model 即可。
...
A: 1) 下载 macOS .dmg 装上 2) 启动后会进 onboarding 3) 1 分钟内 3 步走完 4) 看到示例数据 5) 开始用。
