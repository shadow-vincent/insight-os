...
装了但不知道怎么用？App 内的 **操作手册** 是完整的用户文档：

- **[⚡ 5 分钟上手](#)** — 从打开到写第一篇文章（App 内 `/docs/quickstart`）
- **[💡 核心概念](#)** — 资产卡 vs 轻量卡 vs 内核卡 / 主题 / 证据等级（App 内 `/docs/concepts`）
- **[🧠 Insight Kernel 详解](#)** — V1.4 新功能，4 类 × 4 字段 × 3 机制（App 内 `/docs/insight-kernel`）
- **[❓ FAQ + 故障排查](#)** — Gatekeeper / ABI / 数据备份 / 常见错误（App 内 `/docs/faq`）

直接打开 http://localhost:4191/docs 看（dev 模式）或打开 .app 左侧导航"操作手册"。
